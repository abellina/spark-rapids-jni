...
class spark_resource_adaptor final : public rmm::mr::device_memory_resource {
 public:
  ...
  std::shared_ptr<full_thread_state> get_thread(long const thread_id)
  {
    std::unique_lock<std::mutex> lock(state_mutex);
    auto const thread = threads.find(thread_id);
    if (thread != threads.end()) { return thread->second; }
    return nullptr;
  }
  ...
  void start_retry_block(long const thread_id)
  {
    ...
    if (thread != nullptr) { thread->reset_retry_state(true); }
  }

  void end_retry_block(long const thread_id)
  {
    auto thread = get_thread(thread_id);
    if (thread != nullptr) { thread->reset_retry_state(false); }
  }
                                    ...
                                    std::unordered_set<long> const& task_ids)
  {
    ...
    auto const was_inserted =
      threads.emplace(thread_id, 
        std::make_shared<full_thread_state>(thread_state::THREAD_RUNNING, thread_id));
 ...
 private:
   ...
   */
  void check_before_oom(std::shared_ptr<full_thread_state> state, std::unique_lock<std::mutex> const& lock)
  {
