...
// Macro for logging status with optional notes (supports std::format)
// Uses the global logger instance
// Usage: 
//   LOG_STATUS("OP", thread_id, task_id, state) 
//   LOG_STATUS("OP", thread_id, task_id, state, "simple note")
//   LOG_STATUS("OP", thread_id, task_id, state, "format {} {}", arg1, arg2)
...
// Macro for logging status with label and collection
// Uses the global logger instance
// Usage: LOG_STATUS_COLLECTION("OP", thread_id, task_id, state, label, collection)
#define LOG_STATUS_COLLECTION(op, thread_id, task_id, state, lbl, collection) \
  do { \
    auto _logger = get_logger_if_enabled_status(); \
    if (_logger) { \
      std::stringstream ss; \
...
// Macro for logging status with a container (builds stringstream only if logging is enabled)
// Uses the global logger instance
// Usage: LOG_STATUS_CONTAINER("OP", thread_id, task_id, state, "label", container)
...
// Macro for logging state transitions (supports std::format)
// Uses the global logger instance
// Usage: 
//   LOG_TRANSITION(thread_id, task_id, from_state, to_state)
//   LOG_TRANSITION(thread_id, task_id, from_state, to_state, "simple note")
//   LOG_TRANSITION(thread_id, task_id, from_state, to_state, "format {} {}", arg1, arg2)
#define LOG_TRANSITION(thread_id, task_id, from, to, ...) \
 ...
 // Macro for general info logging
 // Uses the global logger instance
 // Usage: LOG_INFO("message with {} placeholders", arg1, arg2, ...)
 #define LOG_INFO(...) \
 do { \
...
// Uses the global logger instance
// Usage: LOG_DEBUG("message with {} placeholders", arg1, arg2, ...)
#define LOG_DEBUG(...) \
 do { \
...
class spark_resource_adaptor final : public rmm::mr::device_memory_resource {
 public:
  ...
  void start_dedicated_task_thread(long const thread_id, long const task_id)
  {
    ...
    try {
      ...
      // Add to dedicated_threads map if this is a new thread
      if (was_threads_inserted.second == true) {
        dedicated_threads.insert({thread_id, was_threads_inserted.first->second});
      }
    } catch (std::exception const&) {
      if (was_threads_inserted.second == true) {
        // roll back the thread insertion
        dedicated_threads.erase(thread_id);
                                    ...
                                    std::unordered_set<long> const& task_ids)
  {
    ...
    if (was_inserted.second == true) {
      ...
      // Add to pool_threads map
      pool_threads.insert({thread_id, was_inserted.first->second});
 ...
 private:
  ...
  // Map of dedicated task threads (task_id >= 0)
  // Key: thread_id, Value: shared_ptr to the thread state
  std::unordered_map<long, std::shared_ptr<full_thread_state>> dedicated_threads;

  // Map of pool threads (task_id < 0)
  // Key: thread_id, Value: shared_ptr to the thread state
  std::unordered_map<long, std::shared_ptr<full_thread_state>> pool_threads;
      ...
      if (thread_should_be_removed) {
        ...
        switch (threads_at->second->state) {
          ...
          default:
            ...
            // Remove from dedicated_threads or pool_threads map
            if (threads_at->second->task_id >= 0) {
              dedicated_threads.erase(thread_id);
            } else {
              pool_threads.erase(thread_id);
            }
  ...
  /**
   * Returns a map of thread IDs to thread states for threads that are in BUFN (Blocked Until Further Notice) state or above.
    ...
    std::unordered_set<long> const& java_blocked_thread_ids)
  {
    ...
    for (auto const& thread_id : java_blocked_thread_ids) {
      auto const thread_it = threads.find(thread_id);
      if (thread_it != threads.end()) {
        bufn_thread_ids.insert({thread_id, thread_it->second});
      }
    }
                                       ...
                                       std::unordered_set<long> const& java_blocked_thread_ids)
  {
    ...
    for (auto const& [thread_id, t_state] : pool_threads) {
  ...
  void check_and_update_for_bufn_state_machine_only(const std::unique_lock<std::mutex>& lock) {
    check_and_update_for_bufn(lock, java_threads_assumed_running);
  }
    ...
    std::unordered_set<long> const& java_blocked_thread_ids)
  {
    ...
    if (need_to_break_deadlock) {
      ...
      // Since blocked_threads is ordered by priority (highest first), the last entry is the lowest priority
      if (!blocked_threads.empty()) {
        auto const last_blocked = std::prev(blocked_threads.end());
        std::shared_ptr<full_thread_state> thread_to_bufn = last_blocked->second;
        
        if (blocked_threads.size() == 1) {
          ...
          thread_to_bufn->is_retry_alloc_before_bufn = true;
          LOG_STATUS("DETAIL", thread_to_bufn->thread_id, thread_to_bufn->task_id, thread_to_bufn->state,
            "thread (id: {}) is_retry_alloc_before_bufn set to true", thread_to_bufn->thread_id);
          transition(thread_to_bufn, thread_state::THREAD_RUNNING);
        } else {
          log_all_threads_states();
          transition(thread_to_bufn, thread_state::THREAD_BUFN_THROW);
        }
